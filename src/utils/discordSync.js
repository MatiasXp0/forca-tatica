...
const isDiscordConfigured = () => {
  return DISCORD_CONFIG.token && DISCORD_CONFIG.guildId && 
         DISCORD_CONFIG.channels.hierarquia && DISCORD_CONFIG.channels.viaturas &&
         DISCORD_CONFIG.channels.fardamentos && DISCORD_CONFIG.channels.comunicados;
};
...
const discordRequest = async (endpoint, method = 'GET', body = null) => {
  if (!isDiscordConfigured()) {
    console.warn('Discord não configurado. Configure as variáveis de ambiente.');
    return null;
  }
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.error(`Erro Discord API ${response.status}:`, errorText);
      return null;
    }

    if (response.status === 204) {
      return { success: true }; // No content (DELETE)
    }
...
export const upsertDiscordMessage = async (collection, itemId, itemData) => {
  ...
  if (currentMessageId) {
    ...
    if (result) {
      console.log(`✅ ${collection} atualizado no Discord: ${itemData.nome || itemData.titulo}`);
  ...
  if (result && result.id) {
    console.log(`✅ ${collection} criado no Discord: ${itemData.nome || itemData.titulo}`);
...
export const deleteDiscordMessage = async (collection, itemData) => {
  ...
  if (result) {
    console.log(`🗑️ ${collection} removido do Discord: ${itemData.nome || itemData.titulo}`);
    return true;
  }
...
export const initCollectionSync = async (collection, onDataChange) => {
  if (!isDiscordConfigured()) {
    console.warn(`Sincronização de ${collection} desativada - Discord não configurado`);
  ...
  // Enviar log inicial
  await sendDiscordLog(`🔄 Sincronização iniciada para **${collection}**`, 'info');
